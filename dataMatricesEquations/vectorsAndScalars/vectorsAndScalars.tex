...
\begin{example}[Consumer Data]
  ...
  this data as row vectors with the entires running from the most
  popular to the least. We denote these$\vec c_1, \dots, \vec
...
\begin{concept}[Sums via dot product]
...
In general, given an $n$-vector $\vec{v}$, then the average of th
entries of that vector, call it $\mu$, is computed by
  \[
    \mu = \frac{1}{n} \vec{1} \dotp \vec{v}.
  \]
...
\begin{example}[Finding Averages]
  ...
  \begin{explanation}
   ...
   the data, the average can be expressed by
    \begin{align*}
      \mu
      & =  = \frac{1}{\answer[given]{4}} \vec{1} \dotp \vec{s}\\
      &=\frac{1}{\answer[given]{4}}
        \underbrace{
        \begin{pmatrix}
        ...
        \underbrace{
        \begin{pmatrix}
          \answer[given]{90} \\ \answer[given]{85} \\ \answer[given]{100} \\ \answer[given]{80}
        \end{pmatrix}}_{=\vec{s}}\\
        &=\answer[given]{88.75}
    \end{align*}
  \end{explanation}
...
You can think of the dot product as measuring ``how aligned'' two
vectors are. If the dot product is a large positive number, then the
vectors are ``somehow aligned.'' If the dot product is a large
negative number, then the vectors are in ``opposite'' directions. If
the dot product is zero, then the vectors somehow are indepenendent of
each other.
\begin{question}
  Compute:
  \[
...
\begin{example}[RGB Color Space]
  ...
  \begin{explanation}
   ...
   \]
    ...
    \begin{center}
      \begin{tikzpicture}
        \definecolor{rgbyellow}{RGB}{180,180,0}
        \definecolor{rgbcyan}{RGB}{0,180,180}
        \definecolor{rgbmagenta}{RGB}{180,0,180}
        \draw[draw=none,fill={rgb:red,1;green,0;blue,0}] (0,0) rectangle (1,1);
        \draw[draw=none,fill=rgbyellow] (1,0) rectangle (2,1);
        \draw[draw=none,fill={rgb:red,0;green,1;blue,0}] (2,0) rectangle (3,1);
        \draw[draw=none,fill=rgbcyan] (3,0) rectangle (4,1);
