...
\section{The determinant of small matrices}
...
\begin{question}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \filldraw[fill=fill1!50!white,draw=penColor]
      (0, 0) -- ++(5,1) -- ++(-4,3) -- cycle;
...
determinants of three $2 \times 2$ matrices, each of which is obtained
from the original matrix by removing the first row and a column at a
time. Such smaller matrices are called \textit{minor matrices} (of the
given matrix).
...
of minor matrices with alternating signs are called the
...
row. Incidentally, the determinant can also be computed using the
...
\begin{question}
  Compute the following using either the definition or the rule of Sarrus.
  \[
  \det
  \begin{pmatrix}
    1 & 4 & 7\\
...
\begin{image}[0.4\textwidth]
  \begin{tikzpicture}
    ...
    \node[above,penColor] at (2.5,-1) {$\vec{e}_1^\transpose$};
    \node[below right,penColor2] at (-1,2.5) {$\vec{e}_2^\transpose$};
...
\subsection{The determinant in higher dimensions}
...
such as the $2 \times 2$ determinant formula, can be used in understanding
computations involving the determinant.
