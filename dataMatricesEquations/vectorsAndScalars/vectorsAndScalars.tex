...
\begin{definition}[vector $p\text{-norm}$]
  Let $p \in [1, \infty)$. The \dfn{$\boldsymbol{p}$-norm}} of
$\vec{v} \in \R^n$ is denoted by $\|\vec{v}\|_{p}$ and is defined by
  \[
    \|\vec{v}\|_{p}
...
\begin{equation*}
  \|\vec{v}\|_{1} = \sum_{i=1}^{n} |v_i|,
  \quad
  \|\vec{v}\|_{2}  = \sqrt{\sum_{i=1}^{n} |v_i|^2}.
\end{equation*}
...
A vector $\vec{u}$ is called a \dnf{unit vector} if $\|\vec{u}\| =
1$. Depending on the norm used, unit vectors will be different. For
instance, below are illustrations of unit disks in $1$-, $2$-, and
...
\begin{definition}
  Two vectors are called \dfn{orthogonal} if the dot product of
