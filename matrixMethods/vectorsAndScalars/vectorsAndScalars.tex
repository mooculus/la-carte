...
\begin{definition}
  \textcolor{blue}{data can be represented by a single vector. I found this sentence confusing since this is just properties of vectors not related to data} We say data is represented by \dfn{vectors} $\vec{v}$ and $\vec{w}$
  if
  \[
  \vec{v}+\vec{w}
  \]
...
seems too abstract, let's give some very concrete examples, \textcolor{blue}{take out comma here} based on
the examples above. However, before we start, we need to give some
notation for vectors.\textcolor{blue}{I don't see notations here}
...
\begin{align*}
  (1,2,3) + (4,5,6) & = \begin{pmatrix} 1+4 & 2+5 & 3+6   \end{pmatrix} = (5,7,9)\\
  ...
  \begin{explanation}
    ...
    \begin{align*}
      ...
      &=\begin{pmatrix}0\\0\\255\end{pmatrix}+ \left(\answer[given]{2/3}\right)\begin{pmatrix}255\\0\\0\end{pmatrix} \\
              &=\textcolor{blue}{wrote twice take out this line} \begin{pmatrix}0\\0\\255\end{pmatrix}+ \left(\answer[given]{2/3}\right)\begin{pmatrix}255\\0\\0\end{pmatrix}\\
