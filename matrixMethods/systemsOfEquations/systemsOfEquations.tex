...
\begin{example} Solve for $x$, $y$, and $z$ in the following system of equations:
...
\end{array}\right),
\]
which we typically write in matrix notation as $A\textbf{x} = B$, with $A$ being the coefficient matrix and $B$ the constant matrix. Note that here $\textbf{x}$ is a column vector representing the variables in the system. We first write one condensed matrix $(A|B)$ as follows,
\[
\left(\begin{array}{ccc|c}
  2 & 3 & 1 & 5 \\
  1 &  -2 & 2 &-9 \\
...
\end{array}\right).
\]
Now we solve the system by traditional methods such as addition (also
referred to as elimination) method and show what each step means for
our matrix. Recall that in addition method one multiplies one equation
by a constant and then adds it to another equation to eliminate one
variable. Since the second equation in the system above has no
coefficient for $x$, it is best to use it as the equation to multiply
by the needed constant to eliminate the $x$ variable in the other two
equations. We will interchange the first and second equations to make
this process easier:
\[
\begin{array}{ccccccc}
     x& -&2y&+&2z&=&-9\\
