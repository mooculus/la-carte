...
\end{array}\right)
\]
Now we will use Gaussian elimination to transform this matrix to row echelon form.
\[
\begin{pmatrix}
   \answer[given]{2/5}&  \answer[given]{3/10} & \answer[given]{0} &\answer[given]{0} &8 \\
  \answer[given]{0}&  \answer[given]{1} & \answer[given]{0} &\answer[given]{-1/4} &\frac{-3}{2}\\
...
\begin{tikzpicture}
  ...
  % Draw the rectangle around the submatrix
  \draw[black!30!white,thick] ([xshift=-1em,yshift=.5em] m-2-2.north west) rectangle ([xshift=0.5em] m-4-5.south east);
\end{tikzpicture}
\]
