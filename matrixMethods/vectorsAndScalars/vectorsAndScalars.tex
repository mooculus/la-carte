...
additional \underline{(pun intended!)} stipulations, summing the types of
...
\item[RGB Color Space] Colors on digital screens are often represented
  ...
  representing no intensity. The color black can be represented by $(0,0,0)$. On the other hand,
  $255$ represents the  maximum intensity, hence  white is  represented by
...
to \textit{scale} them as well. For example, with respect to
populations, you could ask what is $3$ times or half of the population
of a city. In a similar way, \textit{scaling} makes sense for all the
examples above where it makes sense to add data. Regular old numbers
used to scale vectors are called \textit{scalars}.
...
\textcolor{red}{TK: I object to the following definition of the term
  dimension below. The term has a very specific meaning in linear
  algebra describing an attribute of a vector space, not of a
  vector. In applied fields, it is commonly seen that the term
  `length' of a vector is used to mean the number of its
  components. Admittedly, some people find it confusing with the
  notion of Euclidean length (the square root of sum of squares), but
  it is easily remedied once the notion of vector norms are
  introduced. It is okay for me that the course does not introduce
  vector and matrix norms, but I prefer to see phrases such as
  `n-dimensional vector' or `n-vector' to be introduced, instead of
