...
Given two \textbf{unit} vectors, say $\hat{v}$ and $\hat{w}$, the dot
product as measures ``how aligned'' two vectors are. If the dot
product is $1$, then the vectors are ``completely aligned.'' If the
dot product is $-1$ number, then the vectors are in ``exact opposite''
directions. If the dot product is zero, then the vectors are
