...
\item[As an ordered tuple] An ordered pair is just a pair of numbers
  ...
  \]
\item[As a row vector] A row vectored is a lot like an ordered tuple,
  except we do not use a comma to separate entires. Instead, we use a
...
\end{enumerate}
\begin{solution}
  We'll use the properites of vectors to solve this problem.
  \begin{enumerate}
...
\begin{example}[Financial Transactions]
  ...
  \vec{s} = \begin{pmatrix}199 & 249 & 141 & 304 & 251 \end{pmatrix} 
  \]
  with the entires represeting the categories: Science Fiction,
  Fantasy, Mystery, Romance, Historical in that order.  If each book
  costs the bookstore $\$6$ from the wholesaler, and each book sells
  for $\$9$, what is the profit for each category?
  \begin{solution}
...
\section{Matrices store and tranform data}
