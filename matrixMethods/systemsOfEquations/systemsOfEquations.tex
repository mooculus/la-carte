...
proceedure (when using matrices) can be easily done on a computer with
...
\begin{tikzpicture}[every left delimiter/.style={xshift=.5em},every right delimiter/.style={xshift=-.5em},]
  ...
  \end{tikzpicture}
\end{center}
Note, in the algorithm, we don't need to know much about the entires
...
not, in fact, enamorued with this sort of algorithm.  They, like us
...
\end{align*}
And we have solved our system of equations. \textbf{But wait, there's
  more!}  Often the `form' or shape of the matrix in row echelon form
will \textbf{tell us about the solutions, without actually finding the
solutions.}
