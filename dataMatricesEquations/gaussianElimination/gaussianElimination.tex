...
\[
  ...
  \end{array}
\]
%% TK: Though REF is necessarily upper triangular, the converse is not
%% true. I believe REF is not synomymous to upper-triangularity, and
%% juxtaposing REF with triangular form is incorrect and can be
%% misleading. My suggestion is either drop ``triangular form'' all
%% together, or explain the difference between the two.
Some ``forms'' of these matrices are more convenient than others. One
useful form is called \dfn{row echelon form}.
%% TK: If you want to explain the difference between the two,
%% uncomment the paragraph below.
% Note that while a matrix in row echelon form is necessarily upper
% triangular, the converse is not true. Row echelon form imposes
% additional structure on the leading entries of the rows beyond simply
% having zeros below the main diagonal.
...
%% TK: I like how pivots are characterized in the definition below,
%% but according to the current version, the following non-REF is
%% falsely considered REF
%%
%%  ( 0 0 0 )
%%  ( * * * )
%%  ( 0 * * )
%%
%% because it currently doesn't have any restrictions on the entries
%% above pivot elements.
\begin{definition}
  We say a matrix $M$ is in \dfn{row echelon form} if all rows
...
there are \dfn{no solutions}. A system of equations without a solution
is said to be \dfn{inconsistent}.
% This means that the system of equations is \dfn{overdetermined} also
% known as \dfn{inconsistent}. In a case like this, there are \dfn{no
%   solutions.}
%%
%% TK: A system is said to be overdetermined when there are more
%% equations than there are unknowns. An overdetermined system may
%% have a unique solution, infinitely many solution, or no
%% solutions. In other words, an overdetermined system may or may not
%% be consistent.
...
%% TK: The following needs to be REVISED as it is not true, even in
%% its non-subtle description.
\begin{description}
...
%% TK: Below are application scenarios that I cooked up and I suggest
%% we add somewhere in this chapter. This is in the context of
%% analyzing exam scores. Please take a look and feel free to add,
%% remove, or modify parts or all of it as you see fit. In particular,
%% I think the entire example can be reduced by turning some parts
%% into questions, but I wrote out all details in this draft to share
%% with you exactly what I have in mind.
%%
%% I have a couple more ideas of application scenarios (RGB color
%% mixing and streaming service data analysis), but the one below is
%% already lengthy, so I will refrain from developing them for now.
\begin{example}[Figuring out unknown weights using RREF]
