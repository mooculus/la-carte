...
\textit{vector} when it is resonable to add separate data points.
...
Mathematician's have a special word to denote data your able to resonably add: \textit{vectors}.
...
seems too abstract, I assure you that we will be giving some very
concrete examples all along the way. However, before we start, we need
to give some notation for vectors.
...
\begin{definition}
  ...
  \begin{pmatrix} 1 &  2 & 3 \end{pmatrix}
  \]
  We read the first equality as ``the transpose of the row vector $1$,
  $2$, $3$ is the column vector $1$, $2$, $3$.''  Thus, we can use the
  transpose to switch from a row vector to a column vector and vice
  versa as the second equality above shows.
\end{definition}
