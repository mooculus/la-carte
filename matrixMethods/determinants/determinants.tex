...
\begin{image}
  \begin{tikzpicture}
    ...
    \draw[->,ultra thick,dashed,penColor2] (5,1) -- (7,4);
    \draw[->,ultra thick,dashed,penColor] (2,3) -- (7,4);
    \node[above,penColor] at (2.5,-.25) {$\vec{v}$}; %% <a,b>
    \node[below right,penColor2] at (0.25,2) {$\vec{w}$}; %% <c,d>
  ...
  \begin{tikzpicture}
    ...
    \node[above,penColor2] at (2.5,-.25) {$\vec{v}$}; %% <a,b>
    \node[below right,penColor] at (0.25,2) {$\vec{w}$}; %% <c,d>
...
\begin{itemize}
\item multiplying the first row by a scalar $\alpha$, the determinant is
  multiplied by $\alpha$;
