...
Here we introduce the determinants of by small matrices, in particular, $2 \times 2$ and $3 \times 3$ matrices.

\subsection{The determinants of $2 \times 2$ matrices}
\begin{definition}
...
\]
which are the two rows of the matrix in the definition. Here we assume that
the two vectors are not parallel so that they indeed span a
...
\begin{question}
  Think about a geometric interpretation of the determinant of $2 \times 2$
  matrix whose two rows represent parallel vectors. Is your
  interpretation yield the same results as the one obtained by using
  the formula?
\end{question}
...
\subsection{The determinants of $3 \times 3$ matrices}
...
three rows are not co-planar, meaning not all three vectors are lying
on the same plane.
...
direction of the third vector $\vec{w}$. The illustration above
adheres to the right-hand rule (confirm it!), so the determinant
yields the positive volume of the shape.

Usual approaches to compute this volume would require evaluation of
square roots and trigonometric functions, but the determinant formula
provides a very simple alternative. This is a remarkable way to
calculate the volume!


\subsection{From $3 \times 3$ formula to the cofactor expansion}
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
The notable pattern is that the determinants of smaller submatrices
are multiplied by the entries of the first row with alternating signs,
and then they are added together to yield the determinant of the given
matrix.

% This is a special case of the general method of calculating
% determinants called the \textit{cofactor expansion} or the
% \textit{Laplace expansion}. It is general in the sense that
% \begin{itemize}
% \item it can utilize any other rows or columns, other than the first
%   row, to expand the summ;
% \item it can be used to compute the determinant of any sized square
%   matrix.
% \end{itemize}

% Note also the alternation of signs from term to term. The determinants
% of submatrices with alternating signs are called the
% \textit{cofactors} (of the given matrix).

% Because of these, the formula for the determinant given above is often
% called the \textit{cofactor expansion} or the \textit{Laplace
%   expansion}, where the coefficients are the elements of the first
% row.

Incidentally, the determinant can also be computed using the
entries of the first column as coefficients. See below for the
illustration of this alternate method.
...
column, not just the first row or the first column. A natural question
that arises is how the alternating signs of cofactors are determined
when using a different row or column. The following checkerboard
pattern is useful:
...
For instance, to calculate the determinant using the entries on the second row as
coefficients,
\begin{center}
  \begin{tikzpicture}
    \node at (0,0) {$\det
...
The idea of cofactor expansion nicely extends to higher dimensions. We
present the following general result for the completeness of
presentation.

\begin{theorem}[General cofactor expansion]
  Let $n$ be a positive integer, and let $A = (a_{ij})$ be an $n
  \times n$ matrix. Then
  \begin{enumerate}
  \item for any fixed $i \in \{1, 2, \ldots, n\}$, $\det A = \sum_{j=1}^n
    (-1)^{i+j} a_{ij} \lvert A_{ij} \rvert$;
  \item for any fixed $j \in \{1, 2, \ldots, n\}$, $\det A = \sum_{i=1}^n
    (-1)^{i+j} a_{ij} \lvert A_{ij} \rvert$,
  \end{enumerate}
  where $A_{ij}$ is the submatrix obtained by removing the $i$-th row
  and the $j$-th column from $A$.
\end{theorem}


% TODO: Maybe move this paragraph on the rule of Sarrus to the exercise.
Another way to carry out this computation to follow the \textit{rule of
Sarrus}:
...
% TODO: Recap
% - concrete formulas for 2 \times 2 and 3 \times 3 determinants
% - their geometric interpretation
% - generalizability of cofactor expansion to higher dimensions
