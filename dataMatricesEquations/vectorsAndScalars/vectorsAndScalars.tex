...
%% TK: How about we modify the warning above to something like this?
\begin{warning}
...
\begin{concept}[Dot products are sums]
...
\begin{example}[Data Analysis - Finding Averages]
  ...
  \]
  \begin{explanation}
    One way to summarize the student's performance using a
    \textit{single number} is to calculate the average $\mu$ of the four
    scores. This is a fundamental task in data science which involves summation,
  ...
  In general, if $n$ data is stored in a column vector $\vec{s}$, then
