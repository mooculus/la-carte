...
\begin{description}
  \item[$\boldmath \hat{\vec{v}}\dotp \hat{\vec{w}} = 1$] If the dot product is $1$, then the vectors are ``completely
    aligned.''
  \item[$\boldmath \hat{\vec{v}}\dotp \hat{\vec{w}} = -1$] If the dot
    product is $-1$ number, then the vectors are in ``exact opposite''
    directions.
  \item[$\boldmath \hat{\vec{v}}\dotp \hat{\vec{w}} = 0$]If the dot
