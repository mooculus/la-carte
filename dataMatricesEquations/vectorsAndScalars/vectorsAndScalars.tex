...
\item Now supposing that the wind is blowing at $35$ knots in the true
  ...
  \[
    ...
    \end{pmatrix}
    +
    % This second vector did not get multiplied by 3.
    % \begin{pmatrix}
    %   \answer[given]{32.89} \\
    %   \answer[given]{-11.97}
    % \end{pmatrix}
    ...
    \end{pmatrix}
    =
    % The answer is corrected accordingly.
    % \begin{pmatrix}
    %   \answer[given]{-336.41} \\
    %   \answer[given]{53.15}
    % \end{pmatrix}
...
\end{warning}
 ...
 \begin{definition}
   The \dfn{dot product} (or \textit{scalar product} or \textit{inner
...
% A mathematical apparatus which captures the ``length'' information of
% a given input is called a \textit{norm}. In simple terms, a norm is a
% generalization of the absolute value function, namely, it is a
% function which takes a mathematical object such as a number, a vector,
% a matrix, or even a function, and produces a single meaningful number
% about the input. In many ways, the idea of taking something
% complicated, and reducing it to a single number, is the heart of
% mathematics.\index{heart of mathematics}

% In this section, we will focus on three commonly used \textit{vector
%   norms} and their implications in different types of normalization.
...
\begin{definition}[vector $p\text{-norm}$]
  Let $p \in [1, \infty)$. The \dfn{$\boldsymbol{p}$-norm} of a vector
...
\begin{concept}[General vector norms.]
...
\begin{definition}
  A vector $\vec{u}$ is called a \dfn{unit vector in the $p$-norm}
  if $\|\vec{u}\|_p = 1$.
\end{definition}
...
\hat{\vec{v}}_2$.
% This notational convention will be used consistently through the
% remainder of this textbook.
...
\begin{example}[RGB Color Space]
  ...
  \begin{explanation}
    ...
    $2$-norms as red, green, and blue, we first find their respective
    unit vectors corresponding to $\vec{y}$, $\vec{c}$, and $\vec{m}$:
    \begin{equation*}
    ...
    \end{equation*}
    Multiplying each of these by $255$, we obtain (rounded to the nearest whole numbers)
    \begin{equation*}
...
\begin{definition}
  ...
  \dfn{orthogonal} if $\vec{u} \dotp \vec{v} = 0$.
  % They are said to be \dfn{orthonormal} if, in addition,
  % $\|\vec{u}\| = \|\vec{v}\| = 1$.
\end{definition}
...
\]
% Again, the hat indicates that this is a unit vector obtained by a
% normalization. The subscript 1 indicates that the normalization was
% done using the $1$-norm.

\begin{example}[Population Counts]
  Consider the population counts for Ohio, now expressed as a column
  vector:
  \[
    \vec{p}_{\texttt{OH}}
    = (9394878, 1442655, 20442, 268527, 3907, 544866)^\transpose.
  \]
  ...
  \[
    % \left\| \vec{p}_{\mathtt{OH}} \right\|_1 =
  ...
  \[
    % \hat{\vec{p}_{\texttt{OH}}}_1 =
    \frac{\vec{p}_{\texttt{OH}}}{\vec{1}\dotp\vec{p}_{\texttt{OH}}}
    = \left(\answer[given]{0.8}, \answer[given]{0.12}, \answer[given]{0}, \answer[given]{0.02}, \answer[given]{0}, \answer[given]{0.05}\right)^\transpose
  \]
