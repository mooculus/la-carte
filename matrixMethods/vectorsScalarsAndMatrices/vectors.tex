...
\begin{definition}
  ...
  \[
  \begin{pmatrix} 1 &  2 & 3 \end{pmatrix}^transpose =
...
\begin{question}
  Consider $\vec{s} = \begin{pmatrix}141 & 304 & 249 & 199 & 251 \end{pmatrix}$. Compute $\vec{s}^transpose$.
  \begin{prompt}
  \[
  \vec{s}^transpose  = \begin{pmatrix}\answer{141} \\ \answer{304} \\ \answer{249} \\ \answer{199} \\ \answer{251} \end{pmatrix}
  \]
  \end{prompt}
\end{question}
...
\begin{definition}
The dimension of a vector is the number of entries. Each
individual entry of a vector is called a component.
\end{definition}
...
We can also multiply vectors by a scalar (a number), by
...
A matrix is just a big rectangular array of numbers
\[
M =
...
\]
We give the dimensions of a matrix by stating its number of rows
...
$\boldsymbol{(i,j)}${\bf-}entry of the matrix $M$. Sometimes,
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
We'll leave it to the reader to generalize this procedure to $(1\times n)$-matrix by a $(n\times 1)$ matrix.

\begin{question}
  CHECK TO SEE IF THEY CAN GENERALIZE THE PROCEDURE WITH n=4 and n=6 (use spare matrices)
\end{question}
...
\begin{definition}
  ...
  \[
  \vec{v}\dot \vec{w} = \vec{v}^transpose \vec{w}.
  \]
\end{definition}
...
\begin{question}
  ...
  \cdot \begin{pmatrix} 1 & 1 & 2\end{pmatrix}
  \begin{prompt}
    = \answer{0}
  \end{prompt}
  \]
...
\begin{question}
  ...
  \begin{selectAll}
    \choice[correct]{$(\vec{w} \cdot \vec{u} ) \vec{u}$}
    \choice[correct]{$5(\vec{u} +\vec{w}) \cdot {\vec{u}}$}
    \choice{$\vec{w} / \vec{u}$}
    \choice[correct]{$(2,3) \cdot (4,2) + 7$}
    \choice[correct]{$\vec{w} / ( \vec{u} \cdot \vec{u})$}
    \choice{$(1,3) \cdot (-1,2,5)$}
    \choice{$\vec{u}\cdot \vec{v}+\vec{w}$}
  \end{selectAll}
  \begin{hint}
  ...
  \begin{question}
    ...
    \begin{selectAll}
    \choice[correct]{$(\vec{w} \cdot \vec{u} ) \vec{u}$}
    \choice{$5(\vec{u} +\vec{w}) \cdot {\vec{u}}$}
    \choice{$(2,3) \cdot (4,2) + 7$}
    \choice[correct]{$\vec{w} / ( \vec{u} \cdot \vec{u})$}
    \end{selectAll}
  \end{question}
\end{question}
...
We're basically multiplying the like-shaded sections below:
