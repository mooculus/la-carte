...
\item[As an ordered tuple] An ordered pair is just a pair of numbers
  ...
  represent the \link[demographic information]{https://worldpopulationreview.com/states/states-by-race} for Ohio as a vector
...
% Ordinarily, the term dimension is used to describe an attribute of a
% vector space, not of a vector. What do you think about introducing
% such phrases as
%
%   v is an n-dimensional (column/row) vector;
%
% or
%
%   v is a (column/row) n-vector.
...
\begin{explanation}
  ...
  \item To find the combined demographics of Michigan, Ohio, and
    ...
    \]
  % \item To find the population growth of each demographic, we use the scalar, $1.001^2$ to find:
  %   \[
...
\item Suppose a plane starts at $(0,0)$ and its destination is reached
  by following true bearing $280^\circ$ for $3$ hours at $125$ \link[knots]{https://en.wikipedia.org/wiki/Knot_(unit)}.
...
\item Now supposing that the wind blowing at $35$ knots in the true
  ...
  \[
    3\vec{p} + 3 \vec{w} =
    \begin{pmatrix}\answer[given]{-1009.24}\\ \answer[given]{159.44} \end{pmatrix}
  \]
\end{enumerate}
\end{explanation}
\end{example}
...
% Can we include the following quote at the beginning of this section,
% or even at the beginning of this chapter?
\begin{quote}
  Probably no other area of mathematics has been applied in such
  numerous and diverse contexts as the theory of matrices. In
  mechanics, electromagnetics, statistics, economics, operations
