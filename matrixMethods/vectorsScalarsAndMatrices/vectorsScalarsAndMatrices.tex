...
\item[RGB Color Space] Colors on digital screens are often represented
  as vectors in the RGB color space, with each component corresponding
  to the intensity of red, green, and blue light. Adding the values
  for different pixels changes the brightness of each individual component.
...
\item[As a row vector] A row vector is a lot like an ordered tuple,
  ...
  \vec{p} = \begin{pmatrix}
    ...
    \end{array}
  \]
  Above, we suppose that $\vec{p}$ represents the color (using normalized RGB)
  that represents the color  \textit{Sea Foam Green}.
%% \item[As a column vector] A column vector is just like a row vector,
%%   except is it written vertically:
%%   \[
...
\end{bmatrix}
\]
for an ordered-tuple vectors, row vectors, and column vectors. Ordered tuples and
row vectors are more convenient to write in-line (in a sentence), because they are
...
\begin{explanation}
  ...
  \item To find the combined demographics of Michigan, Ohio, and
    Indiana we compute
    \[
    \vec{p}_{\texttt{MI}} + \vec{p}_{\texttt{OH}} + \vec{p}_{\texttt{IN}} = (\answer[given]{22641134},\answer[given]{3434727},\answer[given]{84507},744076,9229,2349428)
    \]
...
\begin{example}[RGB Color Space]
  ...
  \end{pmatrix}
  \]
  When a nitrocellulose lacquer finish ages, it lightens and begins to yellow.
  \begin{explanation}
